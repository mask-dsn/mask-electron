...
export default class Postbox extends Component {
  ...
  handlePost(event) {
    ...
    console.log(
      `Posting data: ${this.state.postContent} and ${this.state.imageHash}`
    );
    axios
      .post(
    ...
    return (
      <section>
        <div>
          ...
          <button className={styles.uploadbtn} onClick={this.togglePopup}>
            Upload Image
          </button>
          {this.state.showImageName ? <h1>Image attached.</h1> : null}
        </div>
        {this.state.showPopup ? (
          <Popup closePopup={this.togglePopup} handleImage={this.handleImage} />
        ) : null}
      </section>
    );
  }
